...
class Reptile {
  ...
  draw() {
    ...
    if (this.Segments.length != 0) {
      ...
      ctx.lineTo(
        (this.Segments[0].x + 2) * scale,
        (this.Segments[0].y + 2) * scale,
      );
    ...
    let cosHead =
      (cursor.x - x1.Segments[x1.Segments.length - 1].x) /
      Math.sqrt(
        (x1.Segments[x1.Segments.length - 1].x - cursor.x) ** 2 +
          (x1.Segments[x1.Segments.length - 1].y - cursor.y) ** 2,
      );
    let sinHead =
      (cursor.y - x1.Segments[x1.Segments.length - 1].y) /
      Math.sqrt(
        (x1.Segments[x1.Segments.length - 1].x - cursor.x) ** 2 +
          (x1.Segments[x1.Segments.length - 1].y - cursor.y) ** 2,
      );
  ...
  follow(target, targetMove) {
    let cos =
      ...
      Math.sqrt(
        (this.Segments[0].x - target.x) ** 2 +
          (this.Segments[0].y - target.y) ** 2,
      );
    let sin =
    ...
    let cosH =
      (this.Segments[this.Segments.length - 1].x - target.x) /
      Math.sqrt(
        (this.Segments[this.Segments.length - 1].x - target.x) ** 2 +
          (this.Segments[this.Segments.length - 1].y - target.y) ** 2,
      );
    let sinH =
      (this.Segments[this.Segments.length - 1].y - target.y) /
      Math.sqrt(
        (this.Segments[this.Segments.length - 1].x - target.x) ** 2 +
          (this.Segments[this.Segments.length - 1].y - target.y) ** 2,
      );
    ...
    if (
      Math.abs(this.Segments[this.Segments.length - 1].x - targetMove.x) >
      20.001
    ) {
      this.Segments[this.Segments.length - 1].x -=
        80 * this.timeFraction * cosH;
    }
    if (
      Math.abs(this.Segments[this.Segments.length - 1].y - targetMove.y) >
      20.001
    ) {
      this.Segments[this.Segments.length - 1].y -=
        80 * this.timeFraction * sinH;
    }
  }
  segmentsFollow(target) {
    for (let i = 0; i < this.Segments.length - 1; i++) {
      let cos =
        (this.Segments[i + 1].x - target.x) /
        Math.sqrt(
          (this.Segments[i + 1].x - target.x) ** 2 +
            (this.Segments[i + 1].y - target.y) ** 2,
        );
      let sin =
        (this.Segments[i + 1].y - target.y) /
        Math.sqrt(
          (this.Segments[i + 1].x - target.x) ** 2 +
            (this.Segments[i + 1].y - target.y) ** 2,
        );
...
function RenderGame(timestamp) {
  ...
  fluctuation.x =
    x1.Segments[x1.Segments.length - 1].x +
    20 *
      (cosCursor * Math.cos(fluctuation.angle) -
        sinCursor * Math.sin(fluctuation.angle));
  fluctuation.y =
    x1.Segments[x1.Segments.length - 1].y +
    20 *
      (sinCursor * Math.cos(fluctuation.angle) +
        Math.sin(fluctuation.angle) * cosCursor);
